    ...
    return (
        <div style={{display: 'flex'}}>
            <div style={{display: 'flex', flex: 1, flexDirection: 'column'}}>
                { isEdit ? <Button title={'I want to edit'} onClick={() => setIsEdit(false)} sx={{width: '100px', height: '50px'}} >수정 취소</Button> :
                    <Button title={'I want to edit'} onClick={() => setIsEdit(true)} sx={{width: '100px', height: '50px'}} >수정하기</Button> }
                <Container sx={{marginBottom: '16px', marginTop: '20px'}}>
                    { isEdit ? <EditPost postCard={dummy_prop} /> : <DetailPost postData={dummy_prop}/> }
                </Container>
