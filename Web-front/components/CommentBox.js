...
export const CommentBox = () => {
    ...
    const [comments, setComments] = useState([
        {id: 1, author: "landiggity", body: "This is my first comment on this forum so don't be a dick", replyComments: ["123", "abc"]},
    ...
    let commentNodes = showComments ? <div className="comment-list">{_getComments()}</div> : <></>;
        ...
        <>
            <div className="comment">
                ...
                <div className="comment-footer">
                    <a href="#" className="comment-footer-delete" onClick={deleteSelf}>Delete Comment</a>
                </div>
            </div>
            <div>
                {replyComments && replyComments.map((reply) => <Reply key={reply} body={reply} /> )}
            </div>
        </>
    );
}
