...
from config.imports import abort, mariadb

# Import tag to check/add tag for the post
from query.tag_query import (add_post_tag, add_tag, delete_all_tags_of_post,
                             get_tag_by_name)
...
def get_post_feed_with_filter(page, order, filter):
    try:
        ...
        if order:
            ...
            query = "SELECT p.*, u.user_is_endorsed, u.user_is_mod, u.user_nickname FROM User u INNER JOIN (SELECT p.post_id, p.user_id, p.post_title, p.post_text, p.post_image, p.post_like_count, p.post_reply_count, p.post_favourite_count, p.post_date FROM Post p INNER JOIN (SELECT DISTINCT post_id FROM Post_Tag WHERE tag_id IN (" + \
                format_string+")) AS pid ON pid.post_id = p.post_id) AS p ON p.user_id = u.user_id ORDER BY post_like_count DESC LIMIT ?, ?"
        else:
            query = "SELECT p.*, u.user_is_endorsed, u.user_is_mod, u.user_nickname FROM User u INNER JOIN (SELECT p.post_id, p.user_id, p.post_title, p.post_text, p.post_image, p.post_like_count, p.post_reply_count, p.post_favourite_count, p.post_date FROM Post p INNER JOIN (SELECT DISTINCT post_id FROM Post_Tag WHERE tag_id IN (" + \
                format_string+")) AS pid ON pid.post_id = p.post_id) AS p ON p.user_id = u.user_id ORDER BY post_date DESC LIMIT ?, ?"
        ...
        # Set up query statement and values
        query = "SELECT COUNT(*) FROM Post p INNER JOIN (SELECT post_id FROM Post_Tag WHERE tag_id IN (" + \
            format_string+")) AS pid ON pid.post_id = p.post_id"
        values = tuple(filter)
        # Fetching count with given filter
        print("Selecting with query", query, " and values ", values)
        cursor.execute(query, values)
